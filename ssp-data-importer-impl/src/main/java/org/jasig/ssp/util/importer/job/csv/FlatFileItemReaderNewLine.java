 ...
 */
public class FlatFileItemReaderNewLine<T> extends AbstractItemCountingItemStreamItemReader<T> implements
        ResourceAwareItemReaderItemStream<T>, InitializingBean {

    private static final Log logger = LogFactory.getLog(FlatFileItemReaderNewLine.class);
    ...
    public FlatFileItemReaderNewLine() {
        setName(ClassUtils.getShortName(FlatFileItemReaderNewLine.class));
    }
    ...
    private Boolean isIncomplete(String line){
        Integer count = StringUtils.countOccurrencesOf(line, "\"");
        if(count % 2 == 0)
            return false;
        return true;
    }
